...
- `mip_heuristic_portfolio` — **dead option**. Still registered by the patch (so `setOptionValue` succeeds) but read by nothing since #91; #93 deletes the registration. Likewise `mip_heuristic_preset="portfolio"` is no longer recognised: it takes the unknown-preset path (warning + individual flags, i.e. seq/det, where it used to mean port/opp). The patch's preset help string still advertises the value — that string is rewritten by #93, not here, because editing `apply_patch.cmake` forces a clean HiGHS source-tree rebuild.
