...
**Observability**: at `log_dev_level=3` seq/det emits one `[Sequential] heur=<name> effort=<N> wall_ms=<X> effort_per_ms=<R>` line per heuristic per solve (see `src/mode_dispatch.cpp`). Portfolio modes emit the analogous `[Portfolio]` / `[FprLpPortfolio]` lines. `bench/parse_highs_log.py` parses all three; `bench/check_effort_drift.py` aggregates `[Sequential]` samples to recalibrate `kWeight*`.
