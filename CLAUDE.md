...
**No backward compatibility with older patch versions.** `apply_patch.cmake` refuses any HiGHS tree whose `mip-heuristics patch version` marker is not the current `PATCH_VERSION`, and does not attempt an in-place upgrade — no retired-identifier probes, no rewriting an older option layout, no stripping a previous solution-source enum set. Bump `PATCH_VERSION` on any change to inserted text and clean-rebuild; that is the whole contract. The same stance applies to the harness: results trees must use the `<config>/seed<N>/` layout, and config names have no aliases.
