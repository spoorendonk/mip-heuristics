...
- `mode_dispatch` — top-level presolve entry point. Reads `mip_heuristic_*` options and always runs the fixed chain via `run_sequential`. One flag, `mip_heuristic_opportunistic`, picks the parallelism strategy:
  ...
  The flag is threaded to all four heuristics (FJ, FPR, LocalMIP, Scylla) and to `fpr_lp`; each picks its epoch-gated or continuous runner. (History: a Thompson-sampling portfolio bandit used to form a 2×2 with this flag. It produced no consistent gain on PLATO mipfeas and was removed in #91; the implementation is in git history at `00c47c0`.)
...
- `mip_heuristic_portfolio` — **removed in #91**, along with the `"portfolio"` value of `mip_heuristic_preset`. Both are gone from `apply_patch.cmake`, so `setOptionValue("mip_heuristic_portfolio", ...)` now returns `kError` and `preset="portfolio"` takes the unknown-preset path (warning + individual flags). Removing them changes the `HighsOptions` struct layout, so `apply_patch.cmake` carries staleness guards that FATAL_ERROR on an in-place-upgraded HiGHS tree — the idempotency sentinel (`mip_heuristic_opportunistic(false)`) is present in both layouts and cannot tell them apart. If you see that error, `rm -rf build/_deps/highs-src build/_deps/highs-subbuild build/CMakeCache.txt` and rebuild.
...
**Observability**: at `log_dev_level=3` seq/det emits one `[Sequential] heur=<name> effort=<N> wall_ms=<X> effort_per_ms=<R>` line per heuristic per solve (see `src/mode_dispatch.cpp`). `bench/parse_highs_log.py` parses it and `bench/check_effort_drift.py` aggregates the samples to recalibrate `kWeight*`. (The `[Portfolio]` / `[FprLpPortfolio]` tags and their parser support were removed with the bandit in #91.)
