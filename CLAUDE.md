...
- `mip_heuristic_fj_effort` / `mip_heuristic_fpr_effort` / `mip_heuristic_local_mip_effort` / `mip_heuristic_scylla_effort` — one effort budget multiplier per **presolve** heuristic (#110), each a double in `[0.0, 1.0]`; budget = `nnz<<12 * (value/0.05)`, derived in `run_sequential` from the model's `nnz`. They are **independent**: raising one heuristic's budget no longer lowers the others'. That is the whole point — the retired shared envelope, split by `kWeight*` constants proportional to geomean `effort_per_ms`, made "what is a good budget for LocalMIP?" unanswerable without confounding it with FPR and Scylla, and left FJ unreachable from any option at all (measured flat to five significant figures across a 20x sweep). **FJ's option sizes one worker's allowance**; the other three size a whole dispatch, which `make_budget` divides by N. The `budget_is_per_worker` flag in `kChain` is the only place that distinction lives (spelled out because `per_worker` in that translation unit already means `HeuristicBudget::per_worker`, a size_t budget rather than a flag). Defaults are derived from the pre-#110 split: fj `0.0125` = `nnz<<10` per worker, exactly vanilla HiGHS's hardcoded single-thread FJ limit; fpr `0.0884`, local_mip `0.1821`, scylla `0.0296` = `0.30 × w/Σw` for the retired weights 2.99 / 6.16 / 1.00, i.e. their 29.5% / 60.7% / 9.9% shares of the old 0.30 envelope **at `suite=all`**. `tests/test_smoke.cpp` pins all four, because they live in `apply_patch.cmake` where nothing else checks them. **They approximate the retired scheme; they do not reproduce it, and no scalar can.** The old envelope handed a heuristic `budget × max(1 − N/(80e), ¼) × w/Σw_enabled`, which depends on two runtime facts a constant cannot see: the worker count `N`, and which *other* heuristics the suite enabled. Dropping the worker-count term makes `suite=all` 1.04x the old budget at `N=1`, 1.33x at `N=6`, 2x at `N=12` and 4x from `N=18` (where the old quarter-floor capped the FJ deduction); dropping the enabled-set term cuts a *single-heuristic* suite — which used to hand its one heuristic the whole envelope — to 0.29x / 0.61x / 0.10x for fpr / local_mip / scylla, which is the configuration a per-heuristic sweep runs in. FJ is the one exact case, at every `N` and every suite. The anchor is a choice, not a constraint: the deviation is smallest at the low worker counts the tests actually run at (1–2 on CI, `(hardware_concurrency()+1)/2` locally), and erring high is the cheap direction once stall thresholds are absolute (#111) — an over-large budget is truncated by a gate that fires, an under-large one is a hard cap nothing recovers. The full accounting lives in `apply_patch.cmake` beside the defaults, with the calibration-facing version in `docs/PARAMETERS.md`. Treat all four as the starting point of #106's calibration, not the result of one. Note: LocalMIP's reported effort folds in the cold-start construction sweep (Phase B greedy sweep from `local_mip_construction.cpp`, charged in coefficient-access units to match `WorkerCtx::effort`) on dispatches where pool and incumbent are both empty; the cheap Phase A column-write loop is excluded from the normal-path total for unit consistency, but a small `ncol` charge is still reported on the degenerate `nrow==0 || max_effort==0` early exit. `effort_per_ms` remains the cross-heuristic throughput comparison (see Observability below); it no longer calibrates anything, since there are no weights left to set.
**Stall thresholds are absolute, never a fraction of a budget** (#111). Every presolve heuristic stops early when improvement-free effort crosses a threshold, at two levels — a runner-level gate over the whole dispatch (`ContinuousLoopState::effort_since_improvement` against `HeuristicBudget::stale`) and a worker-level gate in `WorkerBudgetState`. Three of the four used to spell that threshold `total_budget >> 2`, which cannot bound over-budgeting: doubling the budget doubles the tolerance, so the gate never fires relatively sooner and charged effort tracks the effort option one-for-one across a 20x sweep (measured 13–20x on FJ/FPR/Scylla before the fix, ≤2x after). FeasibilityJump's `nnz << 8` was the exception and is the model. The thresholds are now one `kStallPerNnz<H>` constant per heuristic, in effort units per matrix nonzero, living in that heuristic's own header (`fj.h`, `fpr.h`, `local_mip.h`, `scylla.h`) because only it knows what its effort counter counts — the four are **not** comparable across heuristics. `stall_threshold(nnz, per_nnz, budget)` in `heuristic_common.h` applies one, clamped to the allowance; `run_sequential` sizes the runner-level gate (multiplying by the worker count for FJ, whose constant's scope is per-worker like its effort option) and `make_budget` divides it by N into `worker_stale`. Scylla is the one documented deviation: it takes the dispatch-level `stale` as its worker threshold, because its per-worker counter is charged the PDLP cost already divided by N. **The four values are provisional pending #106** — they were picked to reproduce the pre-#111 gate at each heuristic's shipped default effort, not measured. Two consequences worth knowing: an absolute threshold makes *small* budgets budget-bound (effort at `0.05` rises) while capping large ones, and the gate only fires when the heuristic actually stops producing — LocalMIP keeps improving on every instance bundled with HiGHS, so its effort still tracks the budget there, correctly. `fpr_lp` is out of scope and keeps `worker_budget >> 2`.

- `mip_heuristic_suite` — **the** heuristic selector, default `all`. The value is either one of the two whole-value aliases `off` / `all`, or a **comma-separated list** of the heuristic names `fj`, `fpr`, `local_mip`, `scylla` (#112), so all fifteen non-empty subsets are expressible: `fj,fpr,local_mip` is the composition the recorded PLATO table was measured at. Order is irrelevant, whitespace around a token is trimmed, repeats are harmless. Replaced `mip_heuristic_preset` and the three `mip_heuristic_run_*` bools in #93. It also gates `fpr_lp` (via `heuristics::effective_flags`), so any value not naming `fpr` disables the dive-time heuristic too. **`heuristics::effective_flags` in `src/mode_dispatch.cpp` is the single place the string becomes four booleans** — the dispatcher and `fpr_lp` both call it, and the legal names are `kChain`'s own `name` field rather than a second table, so a suite token cannot drift from the `[Heur] name=<n>` trace it produces. `off` is an alias for the *whole* value and never a token in a list (`fj,off` is rejected): the patch compares `mip_heuristic_suite == "off"` verbatim in two places, so a value that selected nothing without being that exact string would be a heuristic-free run that is *not* the vanilla-equivalent one. `setLocalOptionValue` strips *spaces* — only spaces — from both ends of a string option's value and lower-cases it before storing (the options-file loader strips tabs, newlines and quotes first), which is what makes that exact comparison safe on both sides of the patch boundary: whatever neither strips misses `== "off"` identically in our parse and in the patched tree. HiGHS does not validate the *value*, so an unknown one is accepted by `setOptionValue` and caught at solve time, where the dispatcher warns — **naming the offending token**, because one typo inside an otherwise valid list silently promotes the run to all four — and fails open to all four. An empty value or a stray trailing comma is an unrecognised token, not a silent `off`. `suite=off` restores HiGHS's own standalone FeasibilityJump call site, which makes it a genuine vanilla-equivalent ablation on the patched binary — see the benchmarking note below.
...
- `[Heur] name=<n> phase=<presolve|dive> start_s=<S> end_s=<E> effort=<N> wall_ms=<X> effort_per_ms=<R> found=<0|1>` — same emission site as `[Sequential]`, one alongside each. It carries what `[Sequential]` cannot: *when* the heuristic ran on the solver clock, which side of the patch boundary it ran on, and whether it produced anything. This is the line the per-heuristic budget calibration reads. `[Sequential]` stays alongside it because external tooling parses it.
...
`run_benchmark.py`'s config surface (#96) is exactly one entry of `CONFIG_SUITES` per `mip_heuristic_suite` value, with no aliases — the `patched` alias for `all` was removed along with the rest of the backward-compatibility surface. Since #112 that includes the ten pairs and triples #107 sweeps, which are named with **`+` where the suite value uses `,`** (`fj+fpr` → `fj,fpr`): a comma is legal in a directory name but not free in a generated table or in LaTeX. Subset names list heuristics in chain order so one subset has exactly one spelling (`fpr+fj` is not a config), and `split_config` still partitions on `@e`, which no name contains. **`config_options` raises on an unknown config name**; it used to return `{}`, so `--configs patchd` produced a fully populated, plausible-looking, completely meaningless results tree. `--budget-sweep` crosses each config with values for the effort option(s) that config reads, into `<output>/<config>@e<V>/seed<N>/`, which `analyze_results.py --configs` consumes unchanged (`@` needs no escaping in a path or in LaTeX text mode). `CONFIG_SWEEP_OPTIONS` maps each config to those option names, and is **derived** from `CONFIG_SUITES` via `sweep_options_for_suite` rather than written out beside it: a config sweeps the effort options of exactly the heuristics its suite value names — one for a singleton, two for `fj+fpr`, all four for `all` (there is no shared envelope left for one number to size, so `all@e0.6` is every heuristic at 0.6, not the shipped ratio between them), and none for `vanilla`/`off`, which run no presolve heuristic. Deriving it is what stops the two tables drifting once there are sixteen configs: a config whose suite value names a heuristic `CHAIN_EFFORT_OPTIONS` does not know raises on import, rather than sweeping a subset of what the config runs, and reachability is a property of the option surface rather than a hand-maintained exemption set (#110 replaced `SWEEP_EXEMPT` with it, FJ having been its only non-trivial member). The two unreachable configs pass through a sweep once as anchor rows rather than becoming N identical trees, and an explicit `vanilla@e0.30` raises. `resolve_config` is the single decomposer of a `<base>@e<V>` name and `build_plan` the single place that decomposition's two *consequences* — which binary, which options — are chosen together; choosing them at separate use sites is how a swept name picks one branch's binary and the other branch's options. A run that solved but ignored its configuration is routed to `<inst>.log.err` like a non-solving exit: HiGHS accepts an unknown `mip_heuristic_suite` *value* and `run_presolve` fails open to all four heuristics with a warning and exit 0, so an `off/` tree would otherwise record runs that executed `all`.
