...
- `effort_ledger` — the only place that writes `heuristic_effort_used` or emits the `[Sequential]` line. `charge_presolve` for the chain; `charge_dive` for `fpr_lp`, which additionally depletes the RENS/RINS LP-iteration envelope. Nothing else in `src/` writes an upstream `HighsMipSolverData` counter.
