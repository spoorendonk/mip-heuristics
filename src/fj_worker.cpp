...
struct FjWorker::Impl {
  ...
  Impl(const HighsLogOptions& log_options, int seed, double epsilon,
       double feastol)
...
void FjWorker::reset_staleness() { effort_since_improvement_ = 0; }
...
EpochResult FjWorker::run_epoch(size_t epoch_budget) {
  ...
  if (!initialized_) {
    ...
    impl_ = std::make_unique<Impl>(log_options, static_cast<int>(seed_),
                                   epsilon, feastol);
    ...
    for (HighsInt col = 0; col < model->num_col_; ++col) {
      ...
      impl_->solver.addVar(fjVarType, lower, upper,
    ...
    for (HighsInt row = 0; row < model->num_row_; ++row) {
      ...
      if (hasFiniteLower || hasFiniteUpper) {
        ...
        if (hasFiniteLower) {
          impl_->solver.addConstraint(RowType::Gte, model->row_lower_[row],
                                      row_num_nz, row_index, row_value, 0);
        }
        if (hasFiniteUpper) {
          impl_->solver.addConstraint(RowType::Lte, model->row_upper_[row],
                                      row_num_nz, row_index, row_value, 0);
        }
      }
    }
    ...
    stale_budget_ = std::min(static_cast<size_t>(nnz) << 8,
                             total_budget_ > 0 ? total_budget_ : SIZE_MAX);
  }
  ...
  if (total_effort_ >= total_budget_ ||
      effort_since_improvement_ > stale_budget_) {
    finished_ = true;
  }
