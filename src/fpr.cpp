...
constexpr int kNumSafeStrategies =
    static_cast<int>(sizeof(kSafeLpFreeStrategies) /
                     sizeof(kSafeLpFreeStrategies[0]));
...
constexpr int kNumAllModes =
    static_cast<int>(sizeof(kAllModes) / sizeof(kAllModes[0]));
...
FprWorker::FprWorker(HighsMipSolver &mipsolver, const CscMatrix &csc,
                     SolutionPool &pool, FprStrategyConfig strat,
                     FrameworkMode mode, uint32_t seed)
...
void FprWorker::randomize_config() {
  int s_idx =
      std::uniform_int_distribution<int>(0, kNumSafeStrategies - 1)(rng_);
...
void FprWorker::recompute_var_order() {
  var_order_ =
      compute_var_order(mipsolver_, strat_.var_strategy, rng_, nullptr);
}
...
void run_parallel(HighsMipSolver &mipsolver, size_t max_effort) {
  ...
  const bool minimize = (model->sense_ == ObjSense::kMinimize);
  const int N = std::min(highs::parallel::num_threads(), kMaxFprWorkers);
  ...
  const size_t epoch_budget =
      std::max<size_t>(per_worker / kEpochsPerWorker, 1);
