...
LocalMipWorker::LocalMipWorker(HighsMipSolver &mipsolver, const CscMatrix &csc,
                               SolutionPool &pool, size_t total_budget,
  ...
  while (spent() < epoch_budget && !base_.exhausted(spent())) {
    ...
    if (feasible_mode) {
      ...
      lift_best.score = 0.0;
      {
        HighsInt write = 0;
        for (HighsInt read = 0;
             read < static_cast<HighsInt>(ctx_.lift.positive_list.size());
             ++read) {
      ...
      if (lift_best.var_idx != -1) {
        ctx_.apply_move_with_tabu(lift_best.var_idx, lift_best.new_val, step_,
                                  rng_);
      } else {
      ...
      if (steps_since_improvement_ >= kFeasiblePlateau) {
        ...
        if (feasible_random_walks_done_ < kFeasibleMaxRandomWalks) {
          perturb_solution(ctx_.solution, *ctx_.mipdata, ctx_.integrality,
                           ctx_.col_lb, ctx_.col_ub, ctx_.ncol, rng_);
      ...
      Candidate cand =
          infeasible_step(ctx_, rng_, step_, best_feasible_, best_objective_,
