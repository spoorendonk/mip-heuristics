...
size_t pump_worker(HighsMipSolver &mipsolver, const CscMatrix &csc, SolutionPool &pool,
                   size_t worker_budget, uint32_t seed) {
...
void run_parallel(HighsMipSolver &mipsolver, size_t max_effort) {
    ...
    auto pdlp_iter_cap = static_cast<HighsInt>((max_effort >> 2) / nnz_lp);
    ...
    for (int w = 0; w < kNumFprConfigs; ++w) {
        ...
        var_orders[w] =
            compute_var_order(mipsolver, kFprConfigs[w].strat.var_strategy, rng, nullptr);
    }
    ...
    while (total_effort < max_effort) {
        ...
        if (pdlp_iters == 0) {
            ++pdlp_stall_count;
            if (pdlp_stall_count >= pump::kMaxPdlpStalls) {
                break;
            }
        } else {
            pdlp_stall_count = 0;
        ...
        size_t remaining_budget = max_effort > total_effort ? max_effort - total_effort : 0;
        ...
        highs::parallel::for_each(
            ...
            [&](HighsInt lo, HighsInt hi) {
                for (HighsInt w = lo; w < hi; ++w) {
                    ...
                    cfg.rng_seed_offset = kBaseSeedOffset + static_cast<uint32_t>(w) + K;
                    cfg.hint = x_bar.data();
                    ...
                    cfg.precomputed_var_order_size = static_cast<HighsInt>(var_orders[ci].size());
        ...
        pump::compute_pump_objective(orig_cost, *x_hat, x_bar, integrality, model->col_lower_,
                                     model->col_upper_, alpha_K, cost_scale, ncol, modified_cost);
