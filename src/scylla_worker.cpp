...
EpochResult ScyllaWorker::run_epoch(size_t epoch_budget) {
  ...
  while (epoch.effort < epoch_budget && !base_.exhausted()) {
    ...
    const bool must_force_fresh =
        consecutive_stale_rounds_ >= max_stale_rounds_;
    if (must_force_fresh) {
      ...
      auto try_res = pdlp_.try_solve_or_snapshot(
          modified_cost_, warm_start_col_value_, warm_start_row_dual_,
    ...
    if (fresh) {
      ...
      pump::compute_pump_objective(
          orig_cost, x_hat, x_bar, integrality, model->col_lower_,
          model->col_upper_, alpha_K_, cost_scale_, ncol_, modified_cost_);
