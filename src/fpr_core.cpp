...
// WalkSAT-based constraint repair (paper Fig. 4).
// Modifies solution and lhs_cache in-place.  Returns true if feasible.
static bool walksat_repair(std::vector<double> &solution,
                           std::vector<double> &lhs_cache,
                           const CscMatrix &csc, PropEngine &E,
                           const double *col_lb, const double *col_ub,
                           const double *row_lo, const double *row_hi,
                           HighsInt nrow, double feastol,
                           const FprConfig &cfg, std::mt19937 &rng,
                           size_t &total_effort) {
  ...
  for (HighsInt i = 0; i < nrow; ++i) {
    curr_total_viol += viol(i, lhs_cache[i]);
  }
  double best_total_viol = curr_total_viol;
  ...
  for (HighsInt step = 0; step < repair_budget && !violated.empty(); ++step) {
    if (total_effort >= cfg.max_effort) break;
    ...
    auto move = walksat_select_move(row, solution.data(), lhs_cache.data(),
                                    col_lb, col_ub, E, cfg.repair_noise, rng,
                                    total_effort);
    if (move.var < 0) continue;

    double old_val = solution[move.var];
    double delta_change = move.val - old_val;
    solution[move.var] = move.val;

    total_effort += col_start[move.var + 1] - col_start[move.var];
    for (HighsInt p = col_start[move.var]; p < col_start[move.var + 1]; ++p) {
      HighsInt i2 = col_row[p];
      double old_viol_i2 = viol(i2, lhs_cache[i2]);
      lhs_cache[i2] += col_val[p] * delta_change;
      double new_viol_i2 = viol(i2, lhs_cache[i2]);
      curr_total_viol += new_viol_i2 - old_viol_i2;
    ...
    if (cfg.repair_track_best && curr_total_viol < best_total_viol) {
...
// Greedy 1-opt: shift each integer variable one step toward better objective
// if feasible (paper Section 6).
static void greedy_oneopt(std::vector<double> &solution,
                          std::vector<double> &lhs_cache,
                          const CscMatrix &csc, const double *col_lb,
                          const double *col_ub, const double *row_lo,
                          const double *row_hi, const double *col_cost,
                          const std::vector<HighsVarType> &integrality,
                          HighsInt ncol, bool minimize, double feastol,
                          size_t &total_effort) {
  const auto &col_start = csc.col_start;
  const auto &col_row = csc.col_row;
  const auto &col_val = csc.col_val;
  ...
  for (HighsInt j = 0; j < ncol; ++j) {
    ...
    total_effort += col_start[j + 1] - col_start[j];
