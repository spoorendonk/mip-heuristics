...
HeuristicResult fpr_attempt(HighsMipSolver& mipsolver, const FprConfig& cfg,
  ...
  // Use initial_solution on attempt 0 if provided, otherwise use hint
  const double* hint_for_attempt =
      (attempt_idx == 0 && initial_solution) ? initial_solution : cfg.hint;

  // Initialize solution
  if (attempt_idx == 0 && hint_for_attempt) {
    for (HighsInt j = 0; j < ncol; ++j) {
      double v = hint_for_attempt[j];
      if (is_int(j)) v = std::round(v);
      solution[j] = std::max(col_lb[j], std::min(col_ub[j], v));
    }
  ...
  const bool use_hint = (attempt_idx == 0 && hint_for_attempt != nullptr);
  auto choose_fix_value = [&](HighsInt j) -> double {
    double lo = vs[j].lb;
    double hi = vs[j].ub;

    if (use_hint) {
      double h = hint_for_attempt[j];
      if (is_int(j)) h = std::round(h);
      if (h >= lo - feastol && h <= hi + feastol)
        return std::max(lo, std::min(hi, h));
    }
  ...
  auto fix_variable = [&](HighsInt j, double value) -> bool {
    if (value < vs[j].lb - feastol || value > vs[j].ub + feastol) return false;
    value = std::max(vs[j].lb, std::min(vs[j].ub, value));
    if (is_int(j)) value = std::round(value);
    vs_undo.push_back({j, vs[j]});
    sol_undo.push_back({j, solution[j]});
  ...
  for (HighsInt idx = 0; idx < ncol; ++idx) {
    ...
    if (!propagate()) {
      for (HighsInt u = static_cast<HighsInt>(vs_undo.size()) - 1;
           u >= vs_mark; --u)
        vs[vs_undo[u].first] = vs_undo[u].second;
      vs_undo.resize(vs_mark);
      for (HighsInt u = static_cast<HighsInt>(sol_undo.size()) - 1;
  ...
  result.effort = static_cast<size_t>(ncol);
  if (fix_failed) return result;
  ...
  // Verify feasibility
  for (HighsInt i = 0; i < nrow; ++i) {
    if (is_violated(i, lhs_cache[i])) return result;
  }
