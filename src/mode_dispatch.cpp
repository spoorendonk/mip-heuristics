...
void log_sequential(const HighsLogOptions &log_options, const char *name,
                    size_t effort, double wall_ms) {
  double effort_per_ms = (effort > 0 && wall_ms > 0.0)
                             ? static_cast<double>(effort) / wall_ms
                             : 0.0;
  highsLogDev(
      log_options, HighsLogType::kVerbose,
      "[Sequential] heur=%s effort=%zu wall_ms=%.1f effort_per_ms=%.3f\n", name,
      effort, wall_ms, effort_per_ms);
}
...
bool run_sequential(HighsMipSolver &mipsolver, size_t budget,
                    bool opportunistic, bool fj_on, bool fpr_on, bool lm_on,
                    bool sc_on) {
  ...
  auto alloc = [&](double w) -> size_t {
    return static_cast<size_t>(budget * w / total_weight);
  };
  ...
  auto deadline_hit = [&]() {
    return mipdata->terminatorTerminated() ||
...
bool run_presolve(HighsMipSolver &mipsolver, size_t budget) {
  ...
  if (preset_applied) {
    ...
    if (portfolio) {
      ...
      result = run_sequential(mipsolver, budget, opportunistic, fj_on, fpr_on,
                              lm_on, sc_on);
    }
  ...
  return run_sequential(mipsolver, budget, opportunistic, fj_on, fpr_on, lm_on,
                        sc_on);
}
