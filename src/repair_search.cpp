...
std::pair<Branch, Branch> move_to_disjunction(const PropEngine& E,
                                              const PropEngine& R,
...
void backtrack_best_open(std::vector<RepairNode>& Q) {
  ...
  auto best = std::min_element(
      Q.begin(), Q.end(),
      [](const RepairNode& a, const RepairNode& b) {
...
bool apply_branch_to_r(PropEngine& R, const RepairNode& node) {
  if (node.var < 0) return true;  // root node — no branch
  bool ok;
  if (node.is_fix) {
    ok = R.fix(node.var, node.val);
    if (ok) ok = R.propagate(node.var);
  } else if (node.is_lb) {
    ok = R.tighten_lb(node.var, node.val);
    if (ok) ok = R.propagate(-1);
  } else {
...
bool repair_search(PropEngine& E, std::vector<double>& solution,
  ...
  auto backtrack_sol_lhs = [&](HighsInt s_mark, HighsInt l_mark) {
    for (HighsInt u = static_cast<HighsInt>(sol_undo.size()) - 1; u >= s_mark;
         --u) {
      solution[sol_undo[u].idx] = sol_undo[u].old_val;
    }
    sol_undo.resize(s_mark);
    for (HighsInt u = static_cast<HighsInt>(lhs_undo.size()) - 1; u >= l_mark;
         --u) {
  ...
  Q.push_back({-1, 0.0, true, false,
               E.vs_mark(), E.sol_mark(),
               R.vs_mark(), R.sol_mark(),
               0, 0,
               total_viol});

  HighsInt nodes_visited = 0;

  while (!Q.empty() && nodes_visited < repair_iterations &&
    ...
    Q.push_back({alternative.var, alternative.val, alternative.is_fix,
                 alternative.is_lb, cur_e_vs, cur_e_sol, cur_r_vs, cur_r_sol,
