...
size_t run_epoch_loop(HighsMipSolver &mipsolver,
  ...
  if (stale_budget == 0) stale_budget = budget >> 2;
  ...
  while (total_effort < budget) {
    ...
    for (int w = 0; w < N; ++w) {
      if (workers[w]->finished()) restart_finished(w);
      if (!workers[w]->finished()) all_finished = false;
    }
    ...
    if (any_improved) {
      ...
      for (int w = 0; w < N; ++w) workers[w]->reset_staleness();
    } else {
