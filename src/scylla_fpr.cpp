...
void run(HighsMipSolver& mipsolver) {
  ...
  // Rank variables by LP fractionality (most fractional first)
  ...
  // Initialize solution from LP relaxation
  ...
  // Pre-allocate propagation worklist and flag arrays
  ...
  // --- Phase 2: Fix & Propagate (single attempt) ---
  for (HighsInt j = 0; j < ncol; ++j) {
    vs[j].lb = col_lb[j];
    vs[j].ub = col_ub[j];
    vs[j].val = 0.0;
  ...
  for (HighsInt j = 0; j < ncol; ++j) {
    ...
    if (!is_integer(j)) {
      if (std::abs(col_cost[j]) > 1e-15) {
        ...
        // Use LP value for continuous variables
        solution[j] = finite_clamp(lp_sol[j], lo, hi);
      }
    } else {
